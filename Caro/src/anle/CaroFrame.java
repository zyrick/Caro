...
public class CaroFrame extends JFrame implements ActionListener {
	...
	private JMenuBar createJMenuBar() {
		...
		String[] help = { "Guidedance", "", "Introduction" };
		mb.add(createJMenu("Guidedance", help, KeyEvent.VK_H));
		return mb;
	}
	...
	private JPanel createPlayerPanel(boolean player) {
		...
		int w = width / 3;
		String imgPlayerO = "yasuo-1.jpg";
		String imgPlayerX = "Zed_Render.png";
	...
	private void actionNewGame() {
		int select = showDialog("Do you want new game?",
				"New Game");
		if (select == 0) {
			scoreO = 0;
			scoreX = 0;
	...
	private void actionNewUnit() {
		int select = showDialog("Do you want new round",
				"New Round");
		if (select == 0) {
			clear();
		}
	...
	private void actionGiveIn() {
		...
		int select = showDialog(playerName + " Want to loose",
				"New Game");
		if (select == 0) {
			scoreO += sO;
			scoreX += sX;
	...
	private void win(int winer) {
		...
		int select = JOptionPane.showOptionDialog(this, "Congratulation! "
